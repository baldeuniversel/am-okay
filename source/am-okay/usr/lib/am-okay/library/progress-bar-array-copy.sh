...
# Overview :
# ---------
# @overview The module (progress-bar-array-copy.sh) is a module that is composed a certain number 
...
function display_progress_bar
{
   # Declaration variables
    ...
    local getPidCommandCpMv=` cat "$a_filePidCommandCpMv" 2> /dev/null `
    ...
    do  
        ...
        #
        if [[ ! ` ps -p "$getPidCommandCpMv" | grep -w -- "$getPidCommandCpMv" ` ]]
        then
            # Remove the content of the line
            printf "\033[2K\r"
